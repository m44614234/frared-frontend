    ...
    >
      {data &&
        data?.map((item: any , index:any) => {
          return (
            <div key={item._id} className="card  my-2 p-2 md:p-4 bg-white rounded-2xl drop-shadow-lg">
            <div className="flex justify-between">
              <div className="bg-red-200 rounded-lg px-2 py-1 text-red-500 flex items-center gap-x-1 text-xs">
                <div>{(item?.discount || 0).toLocaleString("fa-IR")}%</div>
                <div>تخفیف</div>
              </div>
            ...
            <div className="image-box mb-6 ">
              <Link href={`/singleProduct/${item?._id}`} className="cursor-pointer">
                <img
                  className="max-w-52 h-[150px] mx-auto md:h-[180px] "
                  src={singleImage ? singleImage[index] : "../assets/image/products/91.jpg"}
                  alt=""
                />
              </Link>
            </div>
              ...
              <div className="bg-gray-100 rounded-xl py-3 px-2 flex flex-col justify-between items-center lg:flex-row">
                ...
                <div>
                  {isInCart(item?._id) ? (
                    ...
                    <button onClick={()=>AddtoCart(item)} className="flex items-center gap-x-1 text-sm py-2 px-2 rounded-lg text-white bg-red-500 hover:bg-red-400 transition shadow-lg shadow-red-500/50">
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width={22}
                        height={22}
